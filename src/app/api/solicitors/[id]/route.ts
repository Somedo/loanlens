...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const { data: solicitor, error } = await supabase.from('solicitors').select('*').eq('id', params.id).single()
...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const { data, error } = await supabase.from('solicitors').update(solicitorData).eq('id', params.id).select().single()
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  try {
    const supabase = await createClient()
    const { error } = await supabase.from('solicitors').delete().eq('id', params.id)
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
