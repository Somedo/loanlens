...
export default async function EditSolicitorPage({ params }: { params: Promise<{ id: string }> }
) {
  const { id } = await params
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  const { data: userData } = await supabase.from('users').select('company_id').eq('id', user?.id).single()
  const { data: solicitor } = await supabase.from('solicitors').select('*').eq('id', id).eq('company_id', userData?.company_id).single()
